...
{
    public class ChineseAlmanac : ChineseLunisolarCalendar
    {
        protected ChineseLunisolarCalendar netCalendar = new ChineseLunisolarCalendar();
        ...
        public int GetChineseEraOfYear(DateTime solarDateTime)
        {
            ...
            if ((DayDifference(solarDateTime.Year, solarDateTime.Month, solarDateTime.Day) + solarDateTime.Hour / 24)
                    < Term(solarDateTime.Year, 3, true) - 1)
            {
                //判断是否过立春
                g -= 1;
            }
        ...
        {
            double gzD = (solarDateTime.Hour < 23)
                                            ? EquivalentStandardDay(solarDateTime.Year, solarDateTime.Month, solarDateTime.Day)
                                            : EquivalentStandardDay(solarDateTime.Year, solarDateTime.Month, solarDateTime.Day) + 1;
        ...
        {
            double v = 12 * Gan(GetChineseEraOfDay(solarDateTime)) + Math.Floor((double)((solarDateTime.Hour + 1) / 2)) - 11;
        ...
        {
            string[] lunarHoliDayName = { "小寒", "大寒", "立春", "雨水", "惊蛰", "春分", "清明", "谷雨", "立夏", "小满", "芒种", "夏至", "小暑", "大暑", "立秋", "处暑", "白露", "秋分", "寒露", "霜降", "立冬", "小雪", "大雪", "冬至" };
        ...
        public double GetJulianDay(DateTime solarDateTime)
        {
            ...
            double jt = (solarDateTime.Hour + (solarDateTime.Minute + solarDateTime.Second / 60) / 60) / 24 - 0.5
                                    - TimeZoneInfo.Local.GetUtcOffset(solarDateTime).Hours / 24;
            double jd = (ifG == 1)
                                            ? (EquivalentStandardDay(solarDateTime.Year, solarDateTime.Month, solarDateTime.Day) + 1721425 + jt)
                                            : (EquivalentStandardDay(solarDateTime.Year, solarDateTime.Month, solarDateTime.Day) + 1721425 + jt);
            //儒略日
            return jd;
        }
        ...
        public EclipseTime GetEclipse(DateTime solarDateTime, double timeZone)
        {
            EclipseTime re = new EclipseTime();
            ...
            for (double k = -1; k <= 13; k += 0.5)
            {
                ...
                double f1 = f0 + 1.53058868 * k + fc * Math.Sin(aa) - 0.4068 * Math.Sin(ab) + 0.0021 * Math.Sin(2 * aa)
                                        + 0.0161 * Math.Sin(2 * ab) + 0.0104 * Math.Sin(2 * ac) - 0.0074 * Math.Sin(aa - ab)
                                        - 0.0051 * Math.Sin(aa + ab);
                double j = j0 + 28 * k + f1; //朔或望的等效标准天数及时刻

                //记录当前日期的j值 
                double lunD0 = EquivalentStandardDay(solarDateTime.Year, solarDateTime.Month, solarDateTime.Day) - Math.Floor(j);
                //当前日距朔日的差值
                ...
                if (k == (k1 + 0.5))
                {
                    wangT = Tail(j);
                    wangD = Math.Floor(j)
                                    - (EquivalentStandardDay(solarDateTime.Year, solarDateTime.Month, solarDateTime.Day) - lunD + 1) + 1;
                }
                ...
                if ((lunD == 1 && k == k1) || (lunD == wangD && k == (k1 + 0.5)))
                {
                    if (Math.Abs(Math.Sin(ac)) <= 0.36)
                    {
                        ...
                        double r = 0.207 * Math.Sin(aa) + 0.0024 * Math.Sin(2 * aa) - 0.039 * Math.Sin(ab) + 0.0115 * Math.Sin(2 * ab)
                                           - 0.0073 * Math.Sin(aa + ab) - 0.0067 * Math.Sin(aa - ab) + 0.0117 * Math.Sin(2 * ac);
                        double p = Math.Abs(s * Math.Sin(ac) + r * Math.Cos(ac));
                        double q = 0.0059 + 0.0046 * Math.Cos(aa) - 0.0182 * Math.Cos(ab) + 0.0004 * Math.Cos(2 * ab)
                        ...
                        if (p - q <= 1.5572)
                        {
                            ...
                            if (k != Math.Floor(k))
                            {
                                if (p + q >= 1.0129)
                                {
                                    re.Phenomena = EclipsePhenomena.PartialEclipseOfTheMoon;
                                } //月偏食
                                else
                                {
                                    re.Phenomena = EclipsePhenomena.CompleteEclipseOfTheMoon;
                                } //月全食
                            }
                        }
                    }
        ...
        {
            string str = "十一二三四五六七八九初十廿三四五六七八九";
            string vstr = str.Substring((int)Math.Floor((double)v / 10) + 10, 1) + str.Substring(v % 10, 1);
            if (v == 10)
            {
                vstr = "初十";
            }
        ...
        private int GetChineseMonth(DateTime solarDateTime)
        {
            int leap = 0;
            int month = netCalendar.GetMonth(solarDateTime);

            //正月不可能闰月
            for (int i = 2; i <= month; i++)
            {
                if (netCalendar.IsLeapMonth(solarDateTime.Year, i))
                {
                    leap = i - 1;
                    break; //一年中最多有一个闰月
                }
        ...
        /// <param name="solarDateTime"></param>
        /// <returns></returns>
        public string ToStringWithChineseMonth(DateTime solarDateTime)
        {
            string[] ChineseMonthName = new string[] { "正", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二" };
            int month = netCalendar.GetMonth(solarDateTime);
            int year = netCalendar.GetYear(solarDateTime);
            int leap = netCalendar.GetLeapMonth(solarDateTime.Year);
            return (leap == month ? "闰" : "") + ChineseMonthName[GetChineseMonth(solarDateTime) - 1] + "月";
        }
        ...
        {
            string[] s = new string[] { "海中金", "炉中火", "大林木", "路旁土", "剑锋金", "山头火", "洞下水", "城墙土", "白腊金", "杨柳木", "泉中水", "屋上土", "霹雷火", "松柏木", "常流水", "沙中金", "山下火", "平地木", "壁上土", "金箔金", "佛灯火", "天河水", "大驿土", "钗钏金", "桑松木", "大溪水", "沙中土", "天上火", "石榴木", "大海水" };
        ...
        public string GetChineseMonthPronoun(DateTime solarDateTime)
        {
            ...
            return s[GetChineseMonth(solarDateTime) - 1];
        }
        ...
        public string GetRiLu(DateTime solarDateTime)
        {
            ...
            if (bsg == 0)
            {
                dayglus = ToStringWithCelestialStem(10) + "命进禄";
            }
            else if (bsg == 2)
            {
                dayglus = ToStringWithCelestialStem(1) + "命进禄";
            }
            else if (bsg == 3)
            {
                dayglus = ToStringWithCelestialStem(2) + "命进禄";
            }
            else if (bsg == 5)
            {
                dayglus = ToStringWithCelestialStem(3) + "," + ToStringWithCelestialStem(5) + "命进禄";
            }
            else if (bsg == 6)
            {
                dayglus = ToStringWithCelestialStem(4) + "," + ToStringWithCelestialStem(6) + "命进禄";
            }
            else if (bsg == 8)
            {
                dayglus = ToStringWithCelestialStem(7) + "命进禄";
            }
            else if (bsg == 9)
            {
                dayglus = ToStringWithCelestialStem(8) + "命进禄";
            }
            else if (bsg == 11)
            {
                dayglus = ToStringWithCelestialStem(9) + "命进禄";
            }
            else
            {
                dayglus = "";
            ...
            if (dayglus == "")
            {
                return dayglk.Substring(bs, 1) + "命互禄";
            }
        ...
        {
            string[] jcName = new string[] { "建", "除", "满", "平", "定", "执", "破", "危", "成", "收", "开", "闭" };

            int num = Zhi(LM(solarDateTime));
            int num2 = Zhi(LD(solarDateTime)) - num;
        ...
        {
            string[] zrxName = new string[] { "金匮(黄道日)", "天德(黄道日)", "白虎(黑道日)", "玉堂(黄道日)", "天牢(黑道日)", "玄武(黑道日)", "司命(黄道日)", "勾陈(黑道日)", "青龙(黄道日)", "明堂(黄道日)", "天刑(黑道日)", "朱雀(黑道日)" };

            int num = Zhi(LM(solarDateTime));
            int num2 = Zhi(LD(solarDateTime)) - num * 2;
        ...
        {
            int i = Zhi(GetChineseEraOfDay(Utils.Lunar2Solar(year, 1, 1)));
            int t = (15 - i) % 12;
        ...
        /// 获取二十四番花信风。
        /// </summary>
        /// <remarks>所谓花信风，就是指某种节气时开的花,因为是应花期而来的风，所以叫信风。人们挑选一种花期最准确的花为代表，叫做这一节气中的花信风,意即带来开花音讯的风候。</remarks>
        public string GetFolwer(DateTime solarDateTime)
        {
            string re = string.Empty;
            SolarTerm[] solar = GetSolarTerm(solarDateTime);
            int jq = solar[0].SolarTermDateTime.Day;
            int zq = solar[1].SolarTermDateTime.Day;
            int d = netCalendar.GetDayOfMonth(solarDateTime);
            int m = netCalendar.GetMonth(solarDateTime);
            ...
            if (m == 1)
            {
                ...
                else if (d >= jq + 5 && d < jq + 10)
                {
                    re = "山茶";
                }
                else if (d >= jq + 10 && d < zq)
                {
                    re = "水仙";
                }
                else if (d >= zq && d < zq + 5)
        ...
        {
            DateTime dt1 = new DateTime(solarDateTime.Year, solarDateTime.Month,solarDateTime.Day);
            DateTime dt2 = new DateTime(1900,1,31,0,0,0);

            double dayCyclical = dt1.Subtract(dt2).TotalDays + 40;
            return (int)(dayCyclical);
        }
        ...
        private int[] NKyuusei = new int[] { -1, -1, -1 };
        ...
        private int GetTenton(int JD)
        {
            int[] KyuuseiJD =
                    new int[]
                                        {
                                                2404030, 2404600, 2404810, 2408800, 2409010, 2413000, 2413210, 2417200, 2417410, 2421220, 2421400, 2421610, 2425420
                                                , 2425630, 2429620, 2429800, 2430010, 2433820, 2434030, 2438020, 2438230, 2442220, 2442430, 2446420, 2446630,
                                                2450620, 2450830, 2454820, 2455030, 2458840, 2459020, 2459230, 2463250, 2467240, 2467420, 2467630, 2471440, 2471650
                                                , 2475640, 2475850, 2477650
                                        };
            int[] KyuuseiJDF =
                    new int[]
                                        {
                                                1, -3, 1, 7, -9, -3, 1, 7, -9, 7, -3, 1, -3, 1, 7, -3, 1, -3, 1, 7, -9, -3, 1, 7, -9, -3, 1, 7, -9, 7, -3, 1, 1, 7,
                                                -3, 1, -3, 1, 7, -9, -9
                                        };
            int KJD = 0, KJDF = 0;
            int n;
        ...
        /// 获取梅雨。
        /// </summary>
        public string GetMeiyu(DateTime solarDateTime)
        {
            //梅雨
            string re = "";
            int y = solarDateTime.Year;
            int thisD0 = (new GregorianCalendar()).GetDayOfYear(solarDateTime);
        ...
        /// "三九"是指冬至后的第三个九天，约在1月中下旬。"三九"天为什么最冷呢？这要从当时地面吸收和散发热量的多少来看，冬季这时候虽然白昼短，地面吸收的太阳辐射热量最少，但此时地面散发的热量还多于吸收的热量，近地面的空气温度还要继续低下去，当地面吸收到的太阳热量几乎等于地面散发的热量，气温才达到最冷。到"三九"以后，地面吸收的热量又将多于地面散失的热量，近地面的空气温度也随着逐渐回升。因此，一年中最冷的时候一般出现在冬至后的"三九"前后。冬至这一天开始数九，这就是人们所说的“提冬数九”。数上9天是一九，再数9天是二九……数到“九九”就算“九”尽了，“九尽杨花开”，那时天就暖了。
        /// 人说“冷在九、热在伏”，数九虽冷，但由于我国地域辽阔，冷也冷得不一样。黄河中下游的《九九歌》是：一九二九不出手；三九四 九河上走；五九六九沿河望柳；七九开河，八九雁来；九九又一九，耕牛遍地走。 江南的《九九歌》是：一九二九相见弗出手；三九二十七，篱头吹筚篥(古代的一种乐器，意指寒风吹得篱笆噼噼响声)；四九三十六，夜晚如鹭宿(晚上寒冷象白鹤一样卷曲着身体睡眠)；五九四十五，太阳开门户，六九五十四，贫儿争意气；七九六十三，布袖担头担；八九七十二，猫儿寻阳地；九九八十一，犁耙一齐出。最冷的是三九、四九，在吉林：三九四九冻死狗，在江苏则是“三九四九拾粪老汉满街游”，可见气温相差很大。
        /// </remarks>
        public string GetFuJiu(DateTime solarDateTime)
        {
            string re = "";
            ...
            {
                re = String.Format("初伏第{0}天", n);
            }
            if (firsrD0 >= s15 && firsrD0 < s15 + 10)
            {
                re = String.Format("末伏第{0}天", n);
            }
            else
            {
                if (firsrD0 >= s12 + 30 && firsrD0 < s12 + 40)
                {
                    re = String.Format("中伏第{0}天", n);
                }
                if (firsrD0 >= s12 + 40 && firsrD0 < s12 + 50)
                {
                    re = String.Format("中伏第{0}天", n + 10);
                }
            }
            ...
            if (d1 >= 0 || d2 <= 80)
            {
                ...
                re = String.Format("{0}九第{1}天", ToStringWithChineseDay(w).Substring(1, 1), v + 1);
            }
